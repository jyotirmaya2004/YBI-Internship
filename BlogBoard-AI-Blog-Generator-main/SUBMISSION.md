# Project Submission Summary

## Student Details

- Name: Jyotirmaya Behera
- Project: WordSprint - AI Blog Generator

## Project Overview

WordSprint is an AI-powered blogging platform that automates the generation and publishing of technical blog content related to Artificial Intelligence and Machine Learning.

It uses a Python backend workflow to generate articles and a static frontend to display them in a clean category-based website.

## Key Highlights

- Automated blog generation using AI workflows
- Technical categories including ML, DL, NLP, CV, GenAI, Statistics, and AI News
- Static frontend for article browsing
- Metadata and article publishing pipeline
- Ready for local demo execution

## Main Commands

Run the article generation pipeline:

```bash
python backend/run.py
```

Run in dry mode:

```bash
python backend/run.py --dry-run
```

Run AI News generation:

```bash
python backend/run.py --ainews
```

Serve the frontend:

```bash
python -m http.server 8000 --directory frontend
```

## Submission Note

This project has been personalized and prepared for submission by Jyotirmaya Behera.